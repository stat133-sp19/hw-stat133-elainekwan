...
The \(\textbf{Euclidean distance}\) between points \(\textbf{p}\) and \(\textbf{q}\) is the length of the line segment connecting them \((\overline{\textbf{pq}})\).

In Cartesian coordinates, if \(\textbf{p} = (p_{1},p_{2},...,p_{n})\) and \(\textbf{q} = (q_{1},q_{2},...,q_{n})\) are two points in Euclidean \(\textit{n}\)-space, then the distance (d) from \(\textbf{p}\)  to \(\textbf{q}\), or from \(\textbf{q}\) to \(\textbf{p}\) is given by the Pythagorean formula: 
...
The position of a point in a Eculidean \(\textit{n}\)-space is a Euclidean vector. So, \(\textbf{p}\) and \(\textbf{q}\) may be represented as Euclidean vecotrs, starting from the origin of the space (initial point) with their tips (terminal points) ending at the two points. The \(\textbf{Euclidean norm}\), or \(\textbf{Euclidean length}\), or \(\textbf{magnitude}\) of a vector measures the length of the vector:

\[\vert\vert{\textbf{p}}\vert\vert = \sqrt{p_{1}^2 + p_{2}^2 + ... + p_{n}^2} = \sqrt{\textbf{p}\cdot\textbf{p}},\]
